...
NO_OF_TESTS = 10
TEST_DATA_PATH = "test/"
TRAIN_DATA_PATH = "/media/tkal976/Transcend/Tharindu/single_file/Aeye.h5"
...
# def Multi_file_file_handler(folderpath, mode = tf.estimator.ModeKeys.PREDICT):
# 	if(RUN_MODE == tf.estimator.ModeKeys.TRAIN):
# 		for i in range(1,NO_OF_USERS_TRAIN+1):
# 			for j in range(NO_OF_TESTS):
# 				datapath = folderpath + str(i) + "_" + str(j) + ".h5"
# 				X, Y = load_file(datapath)
# 				input_train, labels_train = create_input(X, Y, mode = tf.estimator.ModeKeys.TRAIN)
# 		for i in range(NO_OF_USERS_TRAIN+1,NO_OF_USERS_TRAIN+1+NO_OF_USERS_EVAL):
# 			for j in range(NO_OF_TESTS):
# 				datapath = folderpath + str(i) + "_" + str(j) + ".h5"
# 				X, Y = load_file(datapath)
# 				input_eval, labels_eval = create_input(X, Y, mode = tf.estimator.ModeKeys.TRAIN)
# 		##run the neural netowrk in train mode
# 		run_nn(input_train, labels_train)
# 	else:
...
# 		run_nn(input_array, labels)
# 	return 0

def run_nn(data_sess, value):
...
def cnn_model(X, Y, mode = tf.estimator.ModeKeys.PREDICT):
